...
import 'signature_file_picker_web.dart';

int _signatureDropZoneCounter = 0;

class SignatureDropZone extends StatefulWidget {
  ...
  @override
  State<SignatureDropZone> createState() => _SignatureDropZoneState();
}

class _SignatureDropZoneState extends State<SignatureDropZone> {
  late final String _viewType =
      'signature-drop-zone-${_signatureDropZoneCounter++}';
  ...
  void initState() {
    ...
    ui_web.platformViewRegistry.registerViewFactory(_viewType, (viewId) {
      ...
      root.onClick.listen((_) async {
        ...
        final picked = await pickSignatureFile();
      ...
      root.onDrop.listen((event) async {
        ...
        if (file == null) {
          widget.onError?.call(
            'No se detecto un archivo. Arrastra una imagen PNG, JPG o WEBP.',
          );
          return;
        }
        if (!isSignatureFileNameSupported(file.name)) {
          widget.onError?.call(
            'La firma debe estar en formato PNG, JPG o WEBP.',
          );
          return;
        }

        final picked = await readHtmlSignatureFile(file);
        if (!mounted || picked == null) {
          widget.onError?.call(
            'No se pudo leer la imagen. Intenta con otro archivo.',
          );
          return;
        }
        widget.onFileDropped(picked);
  ...
  @override
  void didUpdateWidget(covariant SignatureDropZone oldWidget) {
  ...
  void _applyStyles() {
    ...
    title.text = hasFile
        ...
        : 'Arrastra aqui tu firma en PNG, JPG o WEBP';
    subtitle.text = hasFile
        ? 'Tambien puedes hacer clic para reemplazarla.'
        : 'Tambien puedes hacer clic aqui para seleccionarla desde tu equipo.';
  }
